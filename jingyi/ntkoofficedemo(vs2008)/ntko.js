...
{ 
    ...
    //TANGER_OCX_OBJ.setIsNoCopy = !bool;
    //TANGER_OCX_OBJ.setEditMenu = !bool;
    //TANGER_OCX_OBJ.setShowRevisions = !bool;
    //TANGER_OCX_OBJ.setMenubar = !bool;
    //TANGER_OCX_OBJ.setToolMenu = !bool;
    //TANGER_OCX_OBJ.setFileSaveAs = !bool;
...
function editoffice(url,newofficetype)
{
	...
	{	
	    var filename=document.all("filename").value;
         //   alert(filename);
	    try { TANGER_OCX_OBJ.BeginOpenFromURL("uploadfile/" + filename); } catch (err) { };
	}
    else
    {
        switch(newofficetype)
