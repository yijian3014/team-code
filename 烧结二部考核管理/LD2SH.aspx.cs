...
public partial class LD2SH : System.Web.UI.Page
{
    ...
    {
        if (Session["UserID"].ToString() == "" || Session["UserID"] == null || Convert.ToInt16(Session["UserID"]) / 1000 !=5)        //判断是否已经登录,若未登录则弹出提示框并返回登录界面.
        {
        ...
        sel_string = "select * from [dzsw].[dbo].SJ2B_KH_KaoHe_info "
             + " where  AppraiseTime between  '"
               ...
               // + " or (flow_state='被考核人'and KHFK_ZT is null and AppraiseGroupID='" + Session["UserID"].ToString() + "')"
                 + " or (flow_state='书记'and   Leader_2_State is null ))"
        ...
        if (rbl_cx.SelectedIndex == 2)
        {
            ...
            + " where  AppraiseTime between  '"
             ...
             // + " or (flow_state<>'被考核人'and KHFK_ZT is not null and AppraiseGroupID='" + Session["UserID"].ToString() + "')"
                + " or (flow_state<>'书记'and   Leader_2_State is not null ))"
           + "  order by AppraiseClass desc, AppraiseGroup,AppraiseTime";
      BTN_BLLC.Visible = false;
        }
    ...
    protected void get_sing_rec(string sel_rec)
    {
        ...
        try
        {
            while (dr.Read())
            {
                ...
                tbx_sjsh_kh_jiner.Text = kh_jiner_;
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        tb1_ld2sp_yj.Text = tb1_ld2sp_yj.Text.Replace("'", "''");
        tb1_ld2sp_yj.Text += "'+ Char(13)+Char(10)+'该信息由:" + Session["UserRname"].ToString() + " 编辑于 " + DateTime.Now.ToString() + "'+Char(13)+Char(10)+'";
        sqlstr_update = "update [dzsw].[dbo].[SJ2B_KH_KaoHe_info] set [Leader_2_Opinion] = '" + tb1_ld2sp_yj.Text
        ...
        if (GridView1.SelectedIndex > -1)
        {
            ...
            if (Flow_State.Text == "书记")
            {
                if (GridView1.Rows.Count > 0)
                {
                    ...
                    if (tbx_Leader_2_Opinion.Text != "&nbsp;")
                        tb1_ld2sp_yj.Text = tbx_Leader_2_Opinion.Text;
                    else
                        tb1_ld2sp_yj.Text = "";
