...
public partial class GDFK : System.Web.UI.Page
{
    ...
    {
        if (Session["UserID"].ToString() == "" || Session["UserID"] == null || Convert.ToInt16(Session["UserID"]) / 1000 != 2)        //判断是否已经登录,若未登录则弹出提示框并返回登录界面.
        {
    ...
    protected void BTN_BLLC_Click(object sender, EventArgs e)
    {
        if (GridView1.SelectedIndex > -1)
        {
            ...
            if ((Convert.ToInt16(lb_AppraiseGroupID.Text) == Convert.ToInt16(login_usrid)) && Flow_State.Text == "考核人")
            {
                //事实上工段这一状态在数据库没有被定义，也就是工段角色没有审核流程的权限，只有反馈的权限。     
                Session["parent_page"] = System.IO.Path.GetFileName(Request.Path).ToString();
                Response.Redirect("KHLR.aspx");
            }


            if (Convert.ToInt16(lb_tcr_usrid.Text) == Convert.ToInt16(login_usrid) && Flow_State.Text == "工段")
            {
                //事实上工段这一状态在数据库没有被定义，也就是工段角色没有审核流程的权限，只有反馈的权限。            
                if (GridView1.Rows.Count > 0)
                {
                    ...
                    if (GridView1.Rows.Count > 0)
                    {
                        ...
                        if (tbx_ClassObjection.Text != "&nbsp;")
                            tbx_gdsh_yj.Text = tbx_ClassObjection.Text;
                        else
                            tbx_gdsh_yj.Text = "";
