...
public partial class DJSH : System.Web.UI.Page
{
    ...
    {
        if (Session["UserID"].ToString() == "" || Session["UserID"] == null || Convert.ToInt16(Session["UserID"]) / 1000 != 1)        //判断是否已经登录,若未登录则弹出提示框并返回登录界面.
        {
  ...
  //      GridView1.DataBind();
    ...
    protected void BTN_BLLC_Click(object sender, EventArgs e)
    {
        if (GridView1.SelectedIndex > -1)
        {
            ...
            //事实上点检这一状态在数据库没有被定义，也就是点检角色没有审核流程的权限，只有发起，反馈的权限。
            if (Convert.ToInt16(lb_tcr_usrid.Text) == Convert.ToInt16(login_usrid) && Flow_State.Text == "点检")
            {
